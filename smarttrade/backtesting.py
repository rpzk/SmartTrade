...
from .smc_indicators import SMCAnalyzer, OrderBlock, FairValueGap, OrderBlockType
...
class BacktestEngine:
    ...
    def test_orderblock_strategy(
        ...
        """
        logger.info(f"Testing Order Block strategy on {symbol} {interval}")
        ...
        if isinstance(order_blocks[0], dict):
            order_blocks = [
                OrderBlock(
            ...
            if current_trade is None or current_trade.is_closed:
                for ob in order_blocks:
                    ...
                    if ob.type == OrderBlockType.BULLISH:
                        distance_to_top = abs(low - ob.top) / ob.top * 100
                        distance_to_bottom = abs(low - ob.bottom) / ob.bottom * 100
                        
                        if distance_to_top <= max_distance_percent or distance_to_bottom <= max_distance_percent:
                            # Entrada COMPRA
                            entry_price = close
                            ...
                            current_trade = Trade(
                                ...
                                metadata={"ob_strength": ob.strength}
                            )
                    ...
                    # Verifica toque em Order Block bearish
                        ...
                        distance_to_top = abs(high - ob.top) / ob.top * 100
                        distance_to_bottom = abs(high - ob.bottom) / ob.bottom * 100
        ...
        return self._calculate_metrics(
            ...
            strategy_name="Order Block Bounce",
            parameters={
